...
def run_main(id_data = 0, setting = "dlvm", n = 1000, p = 10, d=3, d_miwae = 3, h_miwae=128, add_mask=True,
  ...
  in_file_text = setting + "_" + str(n) + "_" + str(p) + "_" + str(d) + "_seed" + str(id) + "_propNA"+"{:3.2f}".format(perc_miss)
  if add_mask:
    out_file_text = setting + "_" + str(n) + "_" + str(p) + "_" + str(d) + "_dmiwae" + str(d_miwae) + "_hmiwae" + str(h_miwae) + "mask_seed" + str(id) + "_propNA"+"{:3.2f}".format(perc_miss)
  else:
    out_file_text = setting + "_" + str(n) + "_" + str(p) + "_" + str(d) + "_dmiwae" + str(d_miwae) + "_hmiwae" + str(h_miwae) + "_seed" + str(id) + "_propNA"+"{:3.2f}".format(perc_miss)
  
  
  data = np.array(pd.read_csv(in_folder+in_file_text+"_xcomp.csv", low_memory=False))[:,1:(p+1)]
  data_miss = np.array(pd.read_csv(in_folder+in_file_text+"_xmiss.csv", low_memory=False))[:,1:(p+1)]
  
  print('data', data.shape)
  print('data_miss', data_miss.shape)
  ...
  #   np.savetxt(out_folder+"/"+out_file_text+"_imp_m"+str(i)+".csv", x_mul_imp_rescaled[i,:,:], delimiter=";")
