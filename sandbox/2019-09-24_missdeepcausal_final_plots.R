...
# configurations <- data.frame(rbind(cbind("nonlinear3", "dlvm2", 5000, 0, 10, FALSE, "_W", "_", "grf", 200, 0, 4),
#                                    cbind("nonlinear3", "dlvm2", 5000, 0.1, 10, FALSE, "_W", "_", "grf", 200, 0, 4),
#                                    cbind("nonlinear3", "dlvm2", 5000, 0.3, 10, FALSE, "_W", "_", "grf", 200, 0, 4),
#                                    cbind("linear3", "mf", 5000, 0, 10, FALSE, "_W", "_", "grf", 200, 0, 4),
#                                    cbind("linear3", "mf", 5000, 0.1, 10, FALSE, "_W", "_", "grf", 200, 0, 4),
#                                    cbind("linear3", "mf", 5000, 0.3, 10, FALSE, "_W", "_", "grf", 200, 0, 4),
#                                    cbind("nonlinear3", "dlvm2", 10000, 0, 10, FALSE, "_W", "_", "grf", 200, 0, 4),
#                                    cbind("nonlinear3", "dlvm2", 10000, 0.1, 10, FALSE, "_W", "_", "grf", 200, 0, 4),
#                                    cbind("nonlinear3", "dlvm2", 10000, 0.3, 10, FALSE, "_W", "_", "grf", 200, 0, 4),
#                                    cbind("linear3", "mf", 10000, 0.1, 10, FALSE, "_W", "_", "grf", 200, 0, 4)))

configurations <- data.frame(rbind(#cbind("nonlinear3", "dlvm2", 5000, 0, 10, FALSE, "", "_", "grf", 200, 0, 4),
...
for (i in 10:dim(configurations)[1]){
  conf <- configurations[i,]
  print(conf)
  dr.res <- c()
  ipw.res <- c()
  seed_seq <- conf$seed.min:conf$seed.max
  if (i==4) {seed_seq<-c(0,1,3,4)}
  if (i==5) {seed_seq<-c(0,1,3,4)}
  if (i==6) {seed_seq<-c(0,1,3,4)}
  ...
  if (conf$link %in% c("linear2", "linear3", "linear0")){
    dr.res <- dr.res %>%
    ...
    if (conf$forest == "na"){
      dr.res <- dr.res %>%
        filter((as.character(method) %in% c("grf.mf.cv", "grf.miwae", "grf.mia",
                                            "grf.Z_true", "grf.miwae.agg")))
      dr.res$method <- droplevels(dr.res$method)
      if (conf$setting %in% c("mf", "dlvm2", "dlvm3")){
        levels(dr.res$method) <-c("grf.MF", "grf.MDC.process", "grf.Z_true",
                                  "grf.MDC.mi")
      } else {
        levels(dr.res$method) <-c("grf.MF", "grf.MDC.process",  "grf.X_true",
                                  "grf.MDC.mi")
      }
    } else {
      dr.res <- dr.res %>%
        filter((as.character(method) %in% c("grf.mice.wy", "grf.mf.cv", "grf.miwae", "grf.mia",
                                            "grf.Z_true", "grf.miwae.agg")))
      dr.res$method <- droplevels(dr.res$method)
      if (conf$setting %in% c("mf", "dlvm2", "dlvm3")){
        levels(dr.res$method) <-c( "grf.MICE", "grf.MDC.process", "grf.Z_true", "grf.MF", "grf.MIA",
                                  "grf.MDC.mi")
      } else {
        levels(dr.res$method) <-c( "grf.MICE", "grf.MDC.process", "grf.X_true", "grf.MF", "grf.MIA",
                                  "grf.MDC.mi")
      }
    }
  }
  ...
  if (conf$setting == "mf") {y.max <- 1.05; y.min <- 0.95} else {y.max <- 1.15; y.min <- 0.99}
  ggplot(dr.res) +
    ...
    xlab("") +
    ylab("")
  ggsave(paste0(fig.dir, "2019-09-24", "_drago_hopper_",conf$link,"_",conf$setting,
                "_n", conf$n, "_p", p, "_d", d, 
